...
config["model_name"] = "Second_Stage_UNetVAE-bs{}".format(config["batch_size"])  # logger
config["image_shape"] = image_shape
config["activation"] = activation
config["input_shape"] = tuple([config["batch_size"]] + [config["nb_channels"]] + list(config["image_shape"]))
config["result_path"] = os.path.join(config["base_path"], "SecondStage_model", save_folder)   # save models and status files
print(f'Result path is: {config["result_path"]}')
config["saved_model_file"] = config["result_path"] + pth_name
print(f'Saved Model file is {config["saved_model_file"]}')
config["segmentation_map_path"] = config["base_path"] + "/pred/" + segmentation_map_path + "/"
print(f'Segmentation Map is {config["segmentation_map_path"]}')
