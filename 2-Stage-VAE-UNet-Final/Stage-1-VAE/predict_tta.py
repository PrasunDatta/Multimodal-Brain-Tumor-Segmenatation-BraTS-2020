...
print(f"Saved Model path is : {config['saved_model_path']}")
config["prediction_dir"] = os.path.join(config["base_path"], "pred", config["checkpoint_file"].split(".pth")[0])
print(f'Prediction dir: {config["prediction_dir"]}')
...
print(f'test_path is : {config["test_path"]}')
...
if config["predict_from_train_data"]:
    config["test_path"] = os.path.join(config["base_path"], "MICCAI_BraTS2020_TrainingData")
...
def init_model_from_states(config):
    ...
    # print(checkpoint)
    # print(model.load_state_dict(torch.load(config['saved_model_path'])))
...
def predict(name_list, model):
        ...
        for idx, inputs in enumerate(predict_process):
            ...
            for i in range(config["batch_size"]):
                ...
                if file_idx < len(name_list):
                    ...
                    # print(os.path.join(tmp_dir, "flip_{}_{}.npy".format(config["tta_idx"], patient_filename)), output_array[i])
    # after all flips
    if tta:
    ...
    if config["predict_from_test_data"]:
        ...
        print(f'Prediction_dir is : {config["prediction_dir"]}')
    if not os.path.exists(config["prediction_dir"]):
        os.mkdir(config["prediction_dir"])
    for patient_filename in name_list:
        ...
        # print(f'Affine load path is : {os.path.join(config["test_path"], patient_filename, patient_filename +"_t1.nii.gz")}')
        ...
        # print(f'Output image path  is : {os.path.join(config["prediction_dir"], patient_filename + ".nii.gz")}')
        ...
        print(f'probsMap_dir is : {os.path.join(propbsMap_dir, patient_filename + ".npy")}')  
